...
mai=NetCDFFile('outputnew_n.nc','r')
maiir = mai.variables['totalyield'][:,:,:]
mai1=NetCDFFile('outputnew_n.nc','r')
...
maiir_avg=scipy.signal.detrend(maiir[1:35,:,:],axis=0)#1979-2012
mai_avg=scipy.signal.detrend(mai[1:35,:,:],axis=0)
...
for i in range(0,360):
	for j in range(0,720):
	        slope[i,j], intercept[i,j], r_value, p_value, std_err = scipy.stats.linregress(gg,mai[1:35,i,j])
                slope_irr[i,j], intercept_irr[i,j], r_value, p_value, std_err = scipy.stats.linregress(gg,maiir[1:35,i,j])
...
plt.savefig('isamdry_1979_2012usoutputnew_n.jpg',bbox_inches='tight')
