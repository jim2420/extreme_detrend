...
mai=NetCDFFile('maizeflood_n.nc','r')
maiir = mai.variables['denitrification'][:,:,:]
mai1=NetCDFFile('nomaizeflood_n.nc','r')
mai = mai1.variables['denitrification'][:,:,:]
...
for i in range(0,x):
	...
	cou1[i,:,:]=cou[:,:]
maiir= ma.masked_where(ind1!=8.0,maiir)
mai= ma.masked_where(ind1!=8.0,mai)

#maiir= ma.masked_where(cou1>=50000,maiir)
#mai= ma.masked_where(cou1>=50000,mai)
#maiir= ma.masked_where(cou1<0,maiir)
#mai= ma.masked_where(cou1<0,mai)
...
maiir_avg=maiir[78:112,:,:]#1979-2012
mai_avg=mai[78:112,:,:]
...
##1961 is zero
cmap = plt.cm.RdYlGn
bounds=[-50,-40,-30,-20,-10,0,1,2,3,4,5]
...
fcl,lona1 = shiftgrid(180.5,mai_avg[j,:,:],lona,start=False)
...
plt.savefig('isamwet_1979_2012sseaflood_noflood_denitrification.jpg',bbox_inches='tight')
